...
class CookieJar {
  ...
  absorb(res: Response): void {
    ...
    const raw: string[] = typeof getAll === 'function'
...
async function apiGet(wiki: string, jar: CookieJar, params: Record<string, string>): Promise<Record<string, unknown>> {
...
async function apiPost(wiki: string, jar: CookieJar, params: Record<string, string>): Promise<Record<string, unknown>> {
...
export async function deploy(pages: Page[], opts: DeployOptions): Promise<void> {
  ...
  // Login
  const loginTokenRes = await apiGet(wiki, jar, { action: 'query', meta: 'tokens', type: 'login' });
  const loginToken = (loginTokenRes['query'] as Record<string, Record<string, string>>)['tokens']['logintoken'];
  ...
  const csrf = (csrfRes['query'] as Record<string, Record<string, string>>)['tokens']['csrftoken'];
